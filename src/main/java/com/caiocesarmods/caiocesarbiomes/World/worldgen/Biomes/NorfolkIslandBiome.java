...
public class NorfolkIslandBiome {
    public static final DeferredRegister<Biome> BIOMES
            = DeferredRegister.create(ForgeRegistries.BIOMES, CaioCesarBiomesMod.MOD_ID);

    private static ConfiguredSurfaceBuilder<?> DefaultSurfaceBuilder;
    public static final RegistryObject<Biome> NORFOLK_ISLAND = BIOMES.register("norfolk_island",
            () -> makeNorfolkIslandBiome(() -> ConfiguredSurfaceBuilders.OCEAN_SAND, 0.1f, 0.26f));


    private static Biome makeNorfolkIslandBiome(final Supplier<ConfiguredSurfaceBuilder<?>> surfaceBuilder, float depth, float scale) {
        ...
        return (new Biome.Builder()).precipitation(Biome.RainType.RAIN).category(Biome.Category.BEACH).depth(depth).scale(scale)
                .temperature(0.8F).downfall(0.8F).setEffects((new BiomeAmbience.Builder()).setWaterColor(4445678)
                        .setWaterFogColor(270131).withSkyColor(8103167).withFoliageColor(7842607)
                        .withGrassColor(9551193).setFogColor(13028309)
