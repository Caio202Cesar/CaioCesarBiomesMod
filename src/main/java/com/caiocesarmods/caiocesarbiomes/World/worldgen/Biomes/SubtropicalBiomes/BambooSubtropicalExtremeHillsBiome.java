...
public class BambooSubtropicalExtremeHillsBiome {
public static final DeferredRegister<Biome> BIOMES
        = DeferredRegister.create(ForgeRegistries.BIOMES, CaioCesarBiomesMod.MOD_ID);

private static ConfiguredSurfaceBuilder<?> DefaultSurfaceBuilder;
public static final RegistryObject<Biome> BAMBOO_SUBTROPICAL_EXTREME_HILLS = BIOMES.register("bamboo_subtropical_extreme_hills",
...
private static Biome makeMontainBiome(final Supplier<ConfiguredSurfaceBuilder<?>> surfaceBuilder, float depth, float scale, boolean isHighland) {
    ...
    return (new Biome.Builder()).precipitation(Biome.RainType.RAIN).category(Biome.Category.EXTREME_HILLS).depth(depth).scale(scale)
            .temperature(0.88F).downfall(1.0F).setEffects((new BiomeAmbience.Builder()).setWaterColor(993300)
