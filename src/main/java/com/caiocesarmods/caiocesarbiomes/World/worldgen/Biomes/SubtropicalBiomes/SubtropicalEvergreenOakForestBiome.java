...
public class SubtropicalEvergreenOakForestBiome {
   public static final DeferredRegister<Biome> BIOMES
           = DeferredRegister.create(ForgeRegistries.BIOMES, CaioCesarBiomesMod.MOD_ID);

   private static ConfiguredSurfaceBuilder<?> DefaultSurfaceBuilder;
   public static final RegistryObject<Biome> SUBTROPICAL_EVERGREEN_OAK_FOREST = BIOMES.register("subtropical_evergreen_oak_forest",
   ...
   private static Biome makeSubtropicalEvergreenOakForestBiome(final Supplier<ConfiguredSurfaceBuilder<?>> surfaceBuilder, float depth, float scale) {
      ...
      return (new Biome.Builder()).precipitation(Biome.RainType.RAIN).category(Biome.Category.FOREST).depth(depth).scale(scale)
              .temperature(0.84F).downfall(0.82F).setEffects((new BiomeAmbience.Builder()).setWaterColor(4159204)
