...
public class OilPalmFoliagePlacer extends FoliagePlacer {
    public OilPalmFoliagePlacer(FeatureSpread radius, FeatureSpread offset) {
        super(radius, offset);
    }
    ...
    public static final Codec<OilPalmFoliagePlacer> CODEC = RecordCodecBuilder.create(instance ->
            ...
            ).apply(instance, OilPalmFoliagePlacer::new)
    );

    @Override
    protected FoliagePlacerType<?> getPlacerType() {
        return ModFoliagePlacer.OIL_PALM_FOLIAGE_PLACER.get(); // Register your foliage placer type
    }
    ...
    private void placeLeafAt(IWorldGenerationReader world, BlockPos pos, Set<BlockPos> leaves, MutableBoundingBox boundingBox) {
        if (world.hasBlockState(pos, s -> s.isAir())) {
            world.setBlockState(pos, TreeBlocks.OIL_PALM_LEAVES.get().getDefaultState()
