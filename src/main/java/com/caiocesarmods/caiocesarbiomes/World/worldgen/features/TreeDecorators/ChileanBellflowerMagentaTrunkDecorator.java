...
public class ChileanBellflowerMagentaTrunkDecorator extends TreeDecorator {
    public static final Codec<ChileanBellflowerMagentaTrunkDecorator> CODEC = Codec.unit(ChileanBellflowerMagentaTrunkDecorator::new);
    public static final ChileanBellflowerMagentaTrunkDecorator INSTANCE = new ChileanBellflowerMagentaTrunkDecorator();
        ...
        trunkPositions.forEach((pos) -> {
            if (rand.nextInt(3) > 0) {
                ...
                if (Feature.isAirAt(world, blockpos)) {
                    this.placeVine(world, blockpos, ChileanBellflowerMagenta.EAST, placed, boundingBox);
                }
            }
            if (rand.nextInt(3) > 0) {
                BlockPos blockpos1 = pos.east();
                if (Feature.isAirAt(world, blockpos1)) {
                    this.placeVine(world, blockpos1, ChileanBellflowerMagenta.WEST, placed, boundingBox);
                }
            }
            if (rand.nextInt(3) > 0) {
                BlockPos blockpos2 = pos.north();
                if (Feature.isAirAt(world, blockpos2)) {
                    this.placeVine(world, blockpos2, ChileanBellflowerMagenta.SOUTH, placed, boundingBox);
                }
            }
            if (rand.nextInt(3) > 0) {
                BlockPos blockpos3 = pos.south();
                if (Feature.isAirAt(world, blockpos3)) {
                    this.placeVine(world, blockpos3, ChileanBellflowerMagenta.NORTH, placed, boundingBox);
                }
            }
        });
    }
    ...
    private void placeVine(IWorldGenerationReader world, BlockPos pos, BooleanProperty property, Set<BlockPos> placed, MutableBoundingBox boundingBox) {
        ...
        BlockState vineState = ModPlants.CHILEAN_BELLFLOWER_MAGENTA.get().getDefaultState().with(property, true);
