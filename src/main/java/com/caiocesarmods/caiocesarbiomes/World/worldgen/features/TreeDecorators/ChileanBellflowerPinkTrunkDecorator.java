...
import com.caiocesarmods.caiocesarbiomes.block.custom.Vines.ChileanBellflowerPink;
import com.caiocesarmods.caiocesarbiomes.block.custom.Vines.ChileanBellflowerRed;
...
public class ChileanBellflowerPinkTrunkDecorator extends TreeDecorator {
    public static final Codec<ChileanBellflowerPinkTrunkDecorator> CODEC = Codec.unit(ChileanBellflowerPinkTrunkDecorator::new);
    public static final ChileanBellflowerPinkTrunkDecorator INSTANCE = new ChileanBellflowerPinkTrunkDecorator();

    @Override
    protected TreeDecoratorType<?> getDecoratorType() {
        // Return your registered type. Replace with your custom type if desired.
        return ModTreeDecorators.CHILEAN_BELLFLOWER_PINK_TRUNK_DECORATOR.get();
    }
        ...
        trunkPositions.forEach((pos) -> {
            if (rand.nextInt(3) > 0) {
                ...
                if (Feature.isAirAt(world, blockpos)) {
                    this.placeVine(world, blockpos, ChileanBellflowerPink.EAST, placed, boundingBox);
                }
            }
            if (rand.nextInt(3) > 0) {
                BlockPos blockpos1 = pos.east();
                if (Feature.isAirAt(world, blockpos1)) {
                    this.placeVine(world, blockpos1, ChileanBellflowerPink.WEST, placed, boundingBox);
                }
            }
            if (rand.nextInt(3) > 0) {
                BlockPos blockpos2 = pos.north();
                if (Feature.isAirAt(world, blockpos2)) {
                    this.placeVine(world, blockpos2, ChileanBellflowerPink.SOUTH, placed, boundingBox);
                }
            }
            if (rand.nextInt(3) > 0) {
                BlockPos blockpos3 = pos.south();
                if (Feature.isAirAt(world, blockpos3)) {
                    this.placeVine(world, blockpos3, ChileanBellflowerPink.NORTH, placed, boundingBox);
                }
            }
        });
    }
    ...
    private void placeVine(IWorldGenerationReader world, BlockPos pos, BooleanProperty property, Set<BlockPos> placed, MutableBoundingBox boundingBox) {
        ...
        BlockState vineState = ModPlants.CHILEAN_BELLFLOWER_PINK.get().getDefaultState().with(property, true);
