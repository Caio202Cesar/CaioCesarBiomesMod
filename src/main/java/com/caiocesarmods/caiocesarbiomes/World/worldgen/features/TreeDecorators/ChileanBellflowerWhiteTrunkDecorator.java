...
import com.caiocesarmods.caiocesarbiomes.block.custom.Vines.ChileanBellflowerWhite;
...
public class ChileanBellflowerWhiteTrunkDecorator extends TreeDecorator {
    public static final Codec<ChileanBellflowerWhiteTrunkDecorator> CODEC = Codec.unit(ChileanBellflowerWhiteTrunkDecorator::new);
    public static final ChileanBellflowerWhiteTrunkDecorator INSTANCE = new ChileanBellflowerWhiteTrunkDecorator();

    @Override
    protected TreeDecoratorType<?> getDecoratorType() {
        // Return your registered type. Replace with your custom type if desired.
        return ModTreeDecorators.CHILEAN_BELLFLOWER_WHITE_TRUNK_DECORATOR.get();
    }
        ...
        trunkPositions.forEach((pos) -> {
            if (rand.nextInt(3) > 0) {
                ...
                if (Feature.isAirAt(world, blockpos)) {
                    this.placeVine(world, blockpos, ChileanBellflowerWhite.EAST, placed, boundingBox);
                }
            }
            if (rand.nextInt(3) > 0) {
                BlockPos blockpos1 = pos.east();
                if (Feature.isAirAt(world, blockpos1)) {
                    this.placeVine(world, blockpos1, ChileanBellflowerWhite.WEST, placed, boundingBox);
                }
            }
            if (rand.nextInt(3) > 0) {
                BlockPos blockpos2 = pos.north();
                if (Feature.isAirAt(world, blockpos2)) {
                    this.placeVine(world, blockpos2, ChileanBellflowerWhite.SOUTH, placed, boundingBox);
                }
            }
            if (rand.nextInt(3) > 0) {
                BlockPos blockpos3 = pos.south();
                if (Feature.isAirAt(world, blockpos3)) {
                    this.placeVine(world, blockpos3, ChileanBellflowerWhite.NORTH, placed, boundingBox);
                }
            }
        });
    }
    ...
    private void placeVine(IWorldGenerationReader world, BlockPos pos, BooleanProperty property, Set<BlockPos> placed, MutableBoundingBox boundingBox) {
        ...
        BlockState vineState = ModPlants.CHILEAN_BELLFLOWER_WHITE.get().getDefaultState().with(property, true);
