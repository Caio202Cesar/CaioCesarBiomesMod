...
public class GloriosaLeavesDecorator extends TreeDecorator {
    // Create a unit codec for this decorator.
    public static final Codec<GloriosaLeavesDecorator> CODEC = Codec.unit(GloriosaLeavesDecorator::new);
    public static final GloriosaLeavesDecorator INSTANCE = new GloriosaLeavesDecorator();

    @Override
    protected TreeDecoratorType<?> getDecoratorType() {
        // Return your registered TreeDecoratorType – ensure you've registered it with your mod's modid.
        return ModTreeDecorators.GLORIOSA_LEAVES_DECORATOR.get();
    }
        ...
        leavesPositions.forEach((leafPos) -> {
            if (rand.nextInt(4) == 0) {
                ...
                if (Feature.isAirAt(world, target)) {
                    this.placeVine(world, target, GloriosaLily.EAST, placed, boundingBox);
                }
            }
            if (rand.nextInt(4) == 0) {
                BlockPos target = leafPos.east();
                if (Feature.isAirAt(world, target)) {
                    this.placeVine(world, target, GloriosaLily.WEST, placed, boundingBox);
                }
            }
            if (rand.nextInt(4) == 0) {
                BlockPos target = leafPos.north();
                if (Feature.isAirAt(world, target)) {
                    this.placeVine(world, target, GloriosaLily.SOUTH, placed, boundingBox);
                }
            }
            if (rand.nextInt(4) == 0) {
                BlockPos target = leafPos.south();
                if (Feature.isAirAt(world, target)) {
                    this.placeVine(world, target, GloriosaLily.NORTH, placed, boundingBox);
                }
            }
        });
    }
