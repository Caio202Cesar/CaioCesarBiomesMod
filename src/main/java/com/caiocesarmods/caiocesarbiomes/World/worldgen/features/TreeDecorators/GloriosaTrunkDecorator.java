...
public class GloriosaTrunkDecorator extends TreeDecorator {
    public static final Codec<GloriosaTrunkDecorator> CODEC = Codec.unit(GloriosaTrunkDecorator::new);
    public static final GloriosaTrunkDecorator INSTANCE = new GloriosaTrunkDecorator();

    @Override
    protected TreeDecoratorType<?> getDecoratorType() {
        // Return your registered type. Replace with your custom type if desired.
        return ModTreeDecorators.GLORIOSA_TRUNK_DECORATOR.get();
    }
        ...
        trunkPositions.forEach((pos) -> {
            if (rand.nextInt(3) > 0) {
                ...
                if (Feature.isAirAt(world, blockpos)) {
                    this.placeVine(world, blockpos, GloriosaLily.EAST, placed, boundingBox);
                }
            }
            if (rand.nextInt(3) > 0) {
                BlockPos blockpos1 = pos.east();
                if (Feature.isAirAt(world, blockpos1)) {
                    this.placeVine(world, blockpos1, GloriosaLily.WEST, placed, boundingBox);
                }
            }
            if (rand.nextInt(3) > 0) {
                BlockPos blockpos2 = pos.north();
                if (Feature.isAirAt(world, blockpos2)) {
                    this.placeVine(world, blockpos2, GloriosaLily.SOUTH, placed, boundingBox);
                }
            }
            if (rand.nextInt(3) > 0) {
                BlockPos blockpos3 = pos.south();
                if (Feature.isAirAt(world, blockpos3)) {
                    this.placeVine(world, blockpos3, GloriosaLily.NORTH, placed, boundingBox);
                }
            }
        });
    }
    ...
    private void placeVine(IWorldGenerationReader world, BlockPos pos, BooleanProperty property, Set<BlockPos> placed, MutableBoundingBox boundingBox) {
        ...
        BlockState vineState = ModPlants.GLORIOSA_LILY.get().getDefaultState().with(property, true);
