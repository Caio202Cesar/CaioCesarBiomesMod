...
public class PeppercornVineLeavesDecorator extends TreeDecorator {
    // Create a unit codec for this decorator.
    public static final Codec<PeppercornVineLeavesDecorator> CODEC = Codec.unit(PeppercornVineLeavesDecorator::new);
    public static final PeppercornVineLeavesDecorator INSTANCE = new PeppercornVineLeavesDecorator();

    @Override
    protected TreeDecoratorType<?> getDecoratorType() {
        // Return your registered TreeDecoratorType – ensure you've registered it with your mod's modid.
        return ModTreeDecorators.PEPPERCORN_VINE_LEAVES_DECORATOR.get();
    }
        ...
        leavesPositions.forEach((leafPos) -> {
            if (rand.nextInt(4) == 0) {
                ...
                if (Feature.isAirAt(world, target)) {
                    this.placeVine(world, target, PeppercornVine.EAST, placed, boundingBox);
                }
            }
            if (rand.nextInt(4) == 0) {
                BlockPos target = leafPos.east();
                if (Feature.isAirAt(world, target)) {
                    this.placeVine(world, target, PeppercornVine.WEST, placed, boundingBox);
                }
            }
            if (rand.nextInt(4) == 0) {
                BlockPos target = leafPos.north();
                if (Feature.isAirAt(world, target)) {
                    this.placeVine(world, target, PeppercornVine.SOUTH, placed, boundingBox);
                }
            }
            if (rand.nextInt(4) == 0) {
                BlockPos target = leafPos.south();
                if (Feature.isAirAt(world, target)) {
                    this.placeVine(world, target, PeppercornVine.NORTH, placed, boundingBox);
                }
            }
        });
    }
    ...
    private void placeVine(IWorldGenerationReader world, BlockPos pos, BooleanProperty property, Set<BlockPos> placed, MutableBoundingBox boundingBox) {
        ...
        BlockState vineState = ModBlocks.PEPPERCORN_VINE.get().getDefaultState().with(property, true);
