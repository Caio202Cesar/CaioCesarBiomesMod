...
public class AgathisSapling extends SaplingBlock {
    public AgathisSapling() {
        super(new AgathisTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.AGATHIS_SAPLING.get(), RenderType.getCutout());
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (validTemp || hasRain || isSummerAllowed) {
    ...
    public boolean canGrow(IBlockReader worldIn, BlockPos pos, BlockState state, boolean isClient) {
        ...
        if (tooCold || noRain || !isSummerAllowed(world, pos)) {
            return false;
        }
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (!worldIn.isRemote) {
            ...
            if (!isSummerAllowed(worldIn, pos)) {
                player.sendMessage(
                        new StringTextComponent("Summers are too cold or too hot for this sapling."),
                        player.getUniqueID()
                );
                return ActionResultType.SUCCESS;
            }
    ...
    private static class AgathisTree extends Tree {
        @Nullable
        @Override
