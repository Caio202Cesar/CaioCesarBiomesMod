...
public class AleppoPineSapling extends SaplingBlock {
    public AleppoPineSapling() {
        super(new AleppoPineTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.ALEPPO_PINE_SAPLING.get(), RenderType.getCutout());
    ...
    private static class AleppoPineTree extends Tree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            return TreeFeatures.ALEPPO_PINE;
        }
    }
}
