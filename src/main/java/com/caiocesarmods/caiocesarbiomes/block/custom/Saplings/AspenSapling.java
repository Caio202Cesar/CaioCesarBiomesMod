...
public class AspenSapling extends SaplingBlock {
    public AspenSapling() {
        super(new AspenTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (!worldIn.isRemote) {
            ...
            float minTemp = 0.16f, maxTemp = 0.74f;
    ...
    private static class AspenTree extends Tree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            return TreeFeatures.ASPEN_TREE;
        }
    }
}
