...
public class BlackwoodAcaciaSapling extends SaplingBlock {
    public BlackwoodAcaciaSapling() {
        super(new BrutiaPineTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.BLACKWOOD_ACACIA_SAPLING.get(), RenderType.getCutout());

    }

    //Hardy from zone 8 to 12
    @Override
    ...
    private static class BrutiaPineTree extends Tree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            if (random.nextInt(10) == 0) {
                return TreeFeatures.BLACKWOOD_ACACIA_FANCY_TREE;
            } else {
                return TreeFeatures.BLACKWOOD_ACACIA_TREE;
            }
        }
    }
}
