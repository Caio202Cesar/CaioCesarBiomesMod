...
public class CamphorSapling extends SaplingBlock {
    public CamphorSapling() {
        super(new CamphorTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.CAMPHOR_SAPLING.get(), RenderType.getCutout());
    ...
    private static class CamphorTree extends BigTree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            return TreeFeatures.CAMPHOR_TREE;
        }
        ...
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getHugeTreeFeature(Random rand) {
            if (rand.nextInt(10) == 0) {
                return TreeFeatures.ANCIENT_CAMPHOR_TREE;
            } else {
                return TreeFeatures.BIG_CAMPHOR_TREE;
            }
        }
    }
}
