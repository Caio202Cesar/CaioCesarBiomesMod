...
public class CanaryPineSapling extends SaplingBlock {
    public CanaryPineSapling() {
        super(new CanaryPineSapling.CanaryPineTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.CANARY_PINE_SAPLING.get(), RenderType.getCutout());
        RenderTypeLookup.setRenderLayer(TreeBlocks.POTTED_CANARY_PINE_SAPLING.get(), RenderType.getCutout());
    ...
    private static class CanaryPineTree extends Tree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            if (random.nextInt(10) == 5) {
                return TreeFeatures.CANARY_PINE_TREE_TALL;
            } else {
                return TreeFeatures.CANARY_PINE_TREE;
            }
        }
    }
}
