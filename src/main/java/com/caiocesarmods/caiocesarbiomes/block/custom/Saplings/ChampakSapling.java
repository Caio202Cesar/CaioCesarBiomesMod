...
public class ChampakSapling extends SaplingBlock {
    public ChampakSapling() {
        super(new ChampakTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.CHAMPAK_SAPLING.get(), RenderType.getCutout());
    ...
    private static class ChampakTree extends Tree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            return TreeFeatures.CHAMPAK_TREE;
        }
    }
}
