...
public class CoastLiveOakSapling extends SaplingBlock {
    public CoastLiveOakSapling() {
        super(new CoastLiveOakSapling.LiveOakTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.COAST_LIVE_OAK_SAPLING.get(), RenderType.getCutout());
    }
    ...
    private static class LiveOakTree extends Tree {
        ...
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            if (random.nextInt(10) == 0) {
                return TreeFeatures.COAST_LIVE_OAK_FANCY_TREE;
            } else {
                return TreeFeatures.COAST_LIVE_OAK_TREE;
            }
        }
    }
}
