...
public class CoihueSapling extends SaplingBlock {
    public CoihueSapling() {
        super(new CoihueTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.COIHUE_SAPLING.get(), RenderType.getCutout());
    ...
    private static class CoihueTree extends Tree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
                return TreeFeatures.COIHUE_TREE;
        }
    }
}
