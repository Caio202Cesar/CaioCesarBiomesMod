...
public class CorkOakSapling extends SaplingBlock {
    public CorkOakSapling() {
        super(new CorkOakTree(), AbstractBlock.Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.CORK_OAK_SAPLING.get(), RenderType.getCutout());
        RenderTypeLookup.setRenderLayer(TreeBlocks.POTTED_CORK_OAK_SAPLING.get(), RenderType.getCutout());
        ...
        @Override
            protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
                if (random.nextInt(10) == 0) {
                    return TreeFeatures.CORK_OAK_FANCY_TREE;
                } else {
                    return TreeFeatures.CORK_OAK_TREE;
                }
            }
    }
}
