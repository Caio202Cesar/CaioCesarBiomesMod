...
public class DouglasFirSapling extends SaplingBlock {
    public DouglasFirSapling() {
        super(new DouglasFirSapling.DouglasFirTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
    ...
    //Since it is no the coastal variant, it is hardy between zone 4 and 6.
    @Override
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (!worldIn.isRemote) {
            ...
            float minTemp = 0.4f, maxTemp = 0.69f;
    ...
    private static class DouglasFirTree extends BigTree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
                return TreeFeatures.DOUGLAS_FIR_TREE;
        }
        ...
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getHugeTreeFeature(Random rand) {
            return TreeFeatures.GIANT_DOUGLAS_FIR_TREE;
        }
    }
}
