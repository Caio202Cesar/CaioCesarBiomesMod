...
public class FremontPoplarSapling extends SaplingBlock {
    public FremontPoplarSapling() {
        super(new FremontPoplarTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.FREMONT_POPLAR_SAPLING.get(), RenderType.getCutout());
        RenderTypeLookup.setRenderLayer(TreeBlocks.POTTED_FREMONT_POPLAR_SAPLING.get(), RenderType.getCutout());

    }

    //Hardy to zone 2 to 10
    @Override
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (!worldIn.isRemote) {
            ...
            float minTemp = 0.16f, maxTemp = 0.89f;
    ...
    private static class FremontPoplarTree extends Tree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            return TreeFeatures.FREMONT_POPLAR_TREE;
        }
    }
}
