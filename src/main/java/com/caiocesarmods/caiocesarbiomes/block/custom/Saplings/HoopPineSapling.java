...
public class HoopPineSapling extends SaplingBlock {
    public HoopPineSapling() {
        super(new HoopPineTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.HOOP_PINE_SAPLING.get(), RenderType.getCutout());
    ...
    private static class HoopPineTree extends Tree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
                return TreeFeatures.HOOP_PINE_TREE;
        }
    }
}
