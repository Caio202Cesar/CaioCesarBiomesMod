...
public class IndianCoralSapling extends SaplingBlock {
    public IndianCoralSapling() {
        super(new IndianCoralTree(), AbstractBlock.Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.INDIAN_CORAL_SAPLING.get(), RenderType.getCutout());
        RenderTypeLookup.setRenderLayer(TreeBlocks.POTTED_INDIAN_CORAL_SAPLING.get(), RenderType.getCutout());
    ...
    private static class IndianCoralTree extends Tree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            if (random.nextInt(10) == 0) {
                return TreeFeatures.INDIAN_CORAL_FANCY_TREE;
            } else {
                return TreeFeatures.INDIAN_CORAL_TREE;
            }
        }
