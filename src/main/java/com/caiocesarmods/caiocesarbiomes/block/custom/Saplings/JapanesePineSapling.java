...
public class JapanesePineSapling extends SaplingBlock {
    public JapanesePineSapling() {
        super(new JapanesePineSapling.JapanesePineTree(), AbstractBlock.Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.JAPANESE_PINE_SAPLING.get(), RenderType.getCutout());
        RenderTypeLookup.setRenderLayer(TreeBlocks.POTTED_JAPANESE_PINE_SAPLING.get(), RenderType.getCutout());

    }

    //Hardy to zone 5 to 10 (real life, it may requires care for thrive in its marginal cold zone 5, like oleanders and manaca-da-serra within theirs)
    @Override
    ...
    private static class JapanesePineTree extends Tree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            return TreeFeatures.JAPANESE_PINE_TREE;
        }
    }
}
