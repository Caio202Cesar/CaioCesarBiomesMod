...
public class LillyPillySapling extends SaplingBlock {
    public LillyPillySapling() {
        super(new LillyPillyTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.LILLY_PILLY_SAPLING.get(), RenderType.getCutout());
    ...
    private static boolean isSummerAllowed(World world, BlockPos pos) {
        ...
        return heat == SummerHeat.VERY_HOT || heat == SummerHeat.HOT || heat == SummerHeat.WARM || heat == SummerHeat.MILD;
    }

    private static boolean isSummerAllowedToSapling(World world, BlockPos pos) {
        SummerHeat heat = SummerHeat.fromTemperature(SummerHeatHelper.get(world, pos));
        return heat == SummerHeat.HOT || heat == SummerHeat.WARM || heat == SummerHeat.MILD;
    }
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        boolean isSummerAllowedToSapling = isSummerAllowedToSapling(world, pos);
        ...
        if (!isSummerAllowedToSapling && "SUMMER".equals(currentSeason) && !isSaplingSheltered && random.nextInt(3) == 0) {
            world.setBlockState(pos, Blocks.DEAD_BUSH.getDefaultState());
        }
    }
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (!worldIn.isRemote) {
            ...
            if (!isSummerAllowed(worldIn, pos) && !isProtectedByGlass) {
                player.sendMessage(
                        new StringTextComponent("Summers are too hot or cold for this sapling."),
                        player.getUniqueID()
                );
                return ActionResultType.SUCCESS;
            }
    ...
    private static class LillyPillyTree extends Tree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            return TreeFeatures.LILLY_PILLY_TREE;
        }
    }
}
