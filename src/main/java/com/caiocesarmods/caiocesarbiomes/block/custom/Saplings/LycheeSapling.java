...
public class LycheeSapling extends SaplingBlock {
    public LycheeSapling() {
        super(new LycheeTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.LYCHEE_SAPLING.get(), RenderType.getCutout());
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (!worldIn.isRemote) {
            ...
            float minTemp = 0.85f, maxTemp = 0.89f;
            ...
            if (!isSummerAllowed(worldIn, pos)) {
                player.sendMessage(
                        new StringTextComponent("Summers are too cool or hot for this sapling."),
                        player.getUniqueID()
                );
    ...
    private static class LycheeTree extends Tree {
        @Nullable
        @Override
