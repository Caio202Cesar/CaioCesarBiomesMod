...
public class PersimmonSapling extends SaplingBlock {
    public PersimmonSapling() {
        super(new PersimmonTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.PERSIMMON_SAPLING.get(), RenderType.getCutout());
        RenderTypeLookup.setRenderLayer(TreeBlocks.POTTED_PERSIMMON_SAPLING.get(), RenderType.getCutout());
    }

    //Hardy to zone 4 to 10 (as it is a generic representative for both japanese and american persimmon)
    @Override
    ...
    private static class PersimmonTree extends Tree {
        @Nullable
        @Override
            protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
                if (random.nextInt(10) == 0) {
                    return TreeFeatures.PERSIMMON_FANCY_TREE;
                } else {
                    return TreeFeatures.PERSIMMON_TREE;
                }
            }
    }
}
