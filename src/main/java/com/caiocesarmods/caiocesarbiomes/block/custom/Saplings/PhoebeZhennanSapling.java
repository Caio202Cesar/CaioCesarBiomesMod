...
public class PhoebeZhennanSapling extends SaplingBlock {
    public PhoebeZhennanSapling() {
        super(new PhoebeZhennanTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.PHOEBE_ZHENNAN_SAPLING.get(), RenderType.getCutout());
    ...
    private static class PhoebeZhennanTree extends BigTree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            return TreeFeatures.PHOEBE_ZHENNAN_TREE;
        }
        ...
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getHugeTreeFeature(Random rand) {
            if (rand.nextInt(10) == 0) {
                return TreeFeatures.MEGA_PHOEBE_ZHENNAN_TREE;
            } else {
                return TreeFeatures.MEGA_PHOEBE_ZHENNAN_TREE2;
            }
        }
    }
}
