...
public class PinkIvorySapling extends SaplingBlock {
    public PinkIvorySapling() {
        super(new PinkIvoryTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.PINK_IVORY_SAPLING.get(), RenderType.getCutout());
        RenderTypeLookup.setRenderLayer(TreeBlocks.POTTED_PINK_IVORY_SAPLING.get(), RenderType.getCutout());
    ...
    private static class PinkIvoryTree extends Tree {
        @Nullable
        @Override
            protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
                if (random.nextInt(10) == 0) {
                    return TreeFeatures.PINK_IVORY_FANCY_TREE;
                } else {
                    return TreeFeatures.PINK_IVORY_TREE;
                }
            }
    }
}
