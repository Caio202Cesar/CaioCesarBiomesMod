...
public class PomegranateSapling extends SaplingBlock {
    public PomegranateSapling() {
        super(new PomegranateTree(), AbstractBlock.Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.POMEGRANATE_SAPLING.get(), RenderType.getCutout());
        RenderTypeLookup.setRenderLayer(TreeBlocks.POTTED_POMEGRANATE_SAPLING.get(), RenderType.getCutout());
    }
     ...
     * @param random
     */
    //Hardy to zone 8 (while cold hardy varieties can be grown to zone 6 with protection, such as a sheltered location)
    @Override
    ...
    private static class PomegranateTree extends Tree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            return TreeFeatures.POMEGRANATE_TREE;
        }
    }
}
