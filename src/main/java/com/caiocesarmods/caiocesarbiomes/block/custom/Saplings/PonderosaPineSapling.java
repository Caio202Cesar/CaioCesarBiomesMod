...
public class PonderosaPineSapling extends SaplingBlock {
    public PonderosaPineSapling() {
        super(new PonderosaPineSapling.PonderosaPineTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.PONDEROSA_PINE_SAPLING.get(), RenderType.getCutout());
        RenderTypeLookup.setRenderLayer(TreeBlocks.POTTED_PONDEROSA_PINE_SAPLING.get(), RenderType.getCutout());

    }

    //Hardy from zone 3 to 8
    @Override
    ...
    private static class PonderosaPineTree extends Tree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            if (random.nextInt(10) == 5) {
                return TreeFeatures.PONDEROSA_PINE_TREE_TALL;
            } else {
                return TreeFeatures.PONDEROSA_PINE_TREE;
            }
        }
    }
}
