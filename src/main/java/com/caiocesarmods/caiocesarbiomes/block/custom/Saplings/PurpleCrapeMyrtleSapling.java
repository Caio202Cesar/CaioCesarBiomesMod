...
public class PurpleCrapeMyrtleSapling extends SaplingBlock {
        public PurpleCrapeMyrtleSapling() {
            super(new PurpleCrapeMyrtleTree(), AbstractBlock.Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                    .sound(SoundType.PLANT));
        }

        @OnlyIn(Dist.CLIENT)
        public static void registerRenderLayer() {
            RenderTypeLookup.setRenderLayer(TreeBlocks.PURPLE_CRAPE_MYRTLE_SAPLING.get(), RenderType.getCutout());
            RenderTypeLookup.setRenderLayer(TreeBlocks.POTTED_PURPLE_CRAPE_MYRTLE_SAPLING.get(), RenderType.getCutout());
        }
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (!worldIn.isRemote) {
            ...
            float minTemp = 0.7f, maxTemp = 1.2f;
    ...
    public int getFlammability(BlockState state, IBlockReader world, BlockPos pos, Direction face) {return 80;}

    public int getFireSpreadSpeed(BlockState state, IBlockReader world, BlockPos pos, Direction face) {return 60;}

    private static class PurpleCrapeMyrtleTree extends Tree {
            @Nullable
            @Override
