...
public class RedKapokSapling extends SaplingBlock {
    public RedKapokSapling() {
        super(new RedKapokSapling.RedKapokTree(), AbstractBlock.Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.RED_KAPOK_SAPLING.get(), RenderType.getCutout());
    ...
    private static class RedKapokTree extends Tree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            return TreeFeatures.RED_KAPOK_TREE;
        }
    }
}
