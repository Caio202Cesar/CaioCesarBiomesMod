...
public class RedOakSapling extends SaplingBlock {
    public RedOakSapling() {
        super(new CorkOakTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.RED_OAK_SAPLING.get(), RenderType.getCutout());
        RenderTypeLookup.setRenderLayer(TreeBlocks.POTTED_RED_OAK_SAPLING.get(), RenderType.getCutout());
    }

    //Hardy to zone 3 to 10 (represents several types of oaks with red fall foliage, inclu. Q. rubra, Q. texana, Q. falcata, Q. coccinea, Q. palustris), these species can vary in hardiness to 3 to 9(10)
    @Override
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (!worldIn.isRemote) {
            ...
            float minTemp = 0.3f, maxTemp = 0.89f;
    ...
    private static class CorkOakTree extends Tree {
        @Nullable
        @Override
            protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
                if (random.nextInt(10) == 0) {
                    return TreeFeatures.RED_OAK_FANCY_TREE;
                } else {
                    return TreeFeatures.RED_OAK_TREE;
                }
            }
    }
}
