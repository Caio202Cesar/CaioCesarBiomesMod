...
public class RingCuppedOakSapling extends SaplingBlock {
    public RingCuppedOakSapling() {
        super(new RingCuppedOakTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.RING_CUPPED_OAK_SAPLING.get(), RenderType.getCutout());

    }

    //Hardy to zone 7 to 9
    @Override
    ...
    private static class RingCuppedOakTree extends Tree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            if (random.nextInt(10) == 0) {
                return TreeFeatures.RING_CUPPED_OAK_FANCY_TREE;
            } else {
                return TreeFeatures.RING_CUPPED_OAK_TREE;
            }
        }
    }
}
