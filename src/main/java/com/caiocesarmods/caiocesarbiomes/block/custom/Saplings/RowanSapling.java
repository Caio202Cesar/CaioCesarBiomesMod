...
public class RowanSapling extends SaplingBlock {
    public RowanSapling() {
        super(new RowanSapling.RowanTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.ROWAN_SAPLING.get(), RenderType.getCutout());
        RenderTypeLookup.setRenderLayer(TreeBlocks.POTTED_ROWAN_SAPLING.get(), RenderType.getCutout());
    ...
    //Requires mild summers (hardy from zone 3 to zone 7)
    @Override
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (!worldIn.isRemote) {
            ...
            float minTemp = 0.3f, maxTemp = 0.74f;
    ...
    private static class RowanTree extends Tree {
        @Nullable
        @Override
