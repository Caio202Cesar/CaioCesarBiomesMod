...
public class RubberTreeSapling extends SaplingBlock {
    public RubberTreeSapling() {
        super(new RubberTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.RUBBER_TREE_SAPLING.get(), RenderType.getCutout());
        RenderTypeLookup.setRenderLayer(TreeBlocks.POTTED_RUBBER_TREE_SAPLING.get(), RenderType.getCutout());
    ...
    private static class RubberTree extends Tree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            return TreeFeatures.RUBBER_TREE;
        }
    }
}
