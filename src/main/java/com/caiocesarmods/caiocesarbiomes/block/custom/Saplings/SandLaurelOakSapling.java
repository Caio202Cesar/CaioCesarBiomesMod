...
public class SandLaurelOakSapling extends SaplingBlock {
    public SandLaurelOakSapling() {
        super(new SandLaurelOakTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.SAND_LAUREL_OAK_SAPLING.get(), RenderType.getCutout());
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (!worldIn.isRemote) {
            ...
            float temp = biome.getTemperature(pos);
            float minTemp = 0.65f, maxTemp = 0.84f;
    ...
    private static class SandLaurelOakTree extends Tree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            return TreeFeatures.SAND_LAUREL_OAK_TREE;
        }
    }
}
