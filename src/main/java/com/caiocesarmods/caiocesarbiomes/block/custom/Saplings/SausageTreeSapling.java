...
public class SausageTreeSapling extends SaplingBlock {
    public SausageTreeSapling() {
        super(new SausageTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.SAUSAGE_TREE_SAPLING.get(), RenderType.getCutout());
        RenderTypeLookup.setRenderLayer(TreeBlocks.POTTED_SAUSAGE_TREE_SAPLING.get(), RenderType.getCutout());
    ...
    private static class SausageTree extends Tree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            return TreeFeatures.SAUSAGE_TREE;
        }
    }
}
