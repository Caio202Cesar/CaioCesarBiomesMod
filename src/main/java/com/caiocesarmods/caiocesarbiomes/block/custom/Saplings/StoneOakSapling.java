...
public class StoneOakSapling extends SaplingBlock {
    public StoneOakSapling() {
        super(new StoneOakTree(), Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.STONE_OAK_SAPLING.get(), RenderType.getCutout());
    ...
    private static class StoneOakTree extends Tree {
        @Nullable
        @Override
        protected ConfiguredFeature<BaseTreeFeatureConfig, ?> getTreeFeature(Random random, boolean p_225546_2_) {
            return TreeFeatures.STONE_OAK_TREE;
        }
    }
}
