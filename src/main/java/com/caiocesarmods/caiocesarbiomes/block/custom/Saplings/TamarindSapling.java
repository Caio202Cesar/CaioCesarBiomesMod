...
public class TamarindSapling extends SaplingBlock {
    public TamarindSapling() {
        super(new TamarindSapling.TamarindTree(), AbstractBlock.Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.TAMARIND_SAPLING.get(), RenderType.getCutout());
        RenderTypeLookup.setRenderLayer(TreeBlocks.POTTED_TAMARIND_SAPLING.get(), RenderType.getCutout());
