...
import com.caiocesarmods.caiocesarbiomes.World.worldgen.Climate.SummerTemperature;
...
public class TropicalAlmondSapling extends SaplingBlock {
    public TropicalAlmondSapling() {
        super(new TropicalAlmondTree(), AbstractBlock.Properties.from(Blocks.OAK_SAPLING).hardnessAndResistance(0.0f)
                .sound(SoundType.PLANT));
    }

    @OnlyIn(Dist.CLIENT)
    public static void registerRenderLayer() {
        RenderTypeLookup.setRenderLayer(TreeBlocks.TROPICAL_ALMOND_SAPLING.get(), RenderType.getCutout());
        RenderTypeLookup.setRenderLayer(TreeBlocks.POTTED_TROPICAL_ALMOND_SAPLING.get(), RenderType.getCutout());

    }

    //Hardy to zone 10 (requires hot summers)
    private static boolean isSummerAllowed(World world, BlockPos pos) {
        SummerHeat heat = SummerHeat.fromTemperature(
                SummerTemperature.get(world, pos)
        );
