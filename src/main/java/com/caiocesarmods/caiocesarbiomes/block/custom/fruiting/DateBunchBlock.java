...
public class DateBunchBlock extends Block implements IForgeShearable {
    public DateBunchBlock() {
        super(Properties.from(Blocks.BEEHIVE).hardnessAndResistance(0.2F).tickRandomly()
                .sound(SoundType.WET_GRASS).harvestTool(ToolType.HOE).notSolid().doesNotBlockMovement());
    }
    ...
    public boolean isValidPosition(BlockState state, IWorldReader world, BlockPos pos) {
        ...
        return above.getBlock() == TreeBlocks.DATE_LEAVES.get() || above.getBlock() == TreeBlocks.CANARY_DATE_LEAVES.get();
    }
