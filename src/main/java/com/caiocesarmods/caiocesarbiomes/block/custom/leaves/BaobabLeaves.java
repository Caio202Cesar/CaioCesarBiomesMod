...
public class BaobabLeaves extends LeavesBlock implements IForgeShearable {
    private final Supplier<Block> nextStage;

    public BaobabLeaves(Properties properties, Supplier<Block> nextStage) {
        super(properties);
    ...
    public void randomTick(BlockState state, ServerWorld worldIn, BlockPos pos, Random random) {
        ...
        if (temp >= 0.9F && "WINTER".equals(currentSeason) && nextStage != null && random.nextInt(55) == 0) {

            BlockPos belowPos = pos.down();
            BlockState belowState = worldIn.getBlockState(belowPos);

            if (belowState.isAir()) {
                worldIn.setBlockState(belowPos, TreeBlocks.BAOBAB_STALK.get().getDefaultState(), 2);
            }
        ...
        if (temp <= 0.89F && "SUMMER".equals(currentSeason) && nextStage != null && random.nextInt(55) == 0) {
