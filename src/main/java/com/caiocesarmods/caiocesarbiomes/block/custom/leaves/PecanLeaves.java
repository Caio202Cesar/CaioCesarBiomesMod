...
public class PecanLeaves extends LeavesBlock implements IForgeShearable {
    ...
    Block summerBlock = TreeBlocks.PECAN_FRUITING_LEAVES.get();
    Block fallBlock   = TreeBlocks.PECAN_FALL_LEAVES.get();

    @Override
    public void randomTick(BlockState state, ServerWorld worldIn, BlockPos pos, Random random) {
        String currentSeason = Season.getSeason(worldIn.getDayTime());

        // ---- Retrieve biome and temperature ----
        Biome biome = worldIn.getBiome(pos);
        float baseTemp = biome.getTemperature(); // Base temperature

        // ---- NEW temperature condition ----
        boolean coldEnough = baseTemp < 0.9F;

        // ---- ORIGINAL season + random condition ----
