...
public class WhiteFigLeaves extends LeavesBlock implements IForgeShearable {
    public WhiteFigLeaves(Properties properties) {
        super(properties);
    ...
    public void randomTick(BlockState state, ServerWorld worldIn, BlockPos pos, Random random) {
        ...
        if (temp >= 0.9F && "SUMMER".equals(currentSeason)
            ...
            worldIn.setBlockState(
                    pos,
                    TreeBlocks.WHITE_FIG_WINTER_BRANCHES.get()
                            .getDefaultState()
        ...
        if (temp >= 0.9F && "WINTER".equals(currentSeason)
            ...
            worldIn.setBlockState(
                    pos,
                    TreeBlocks.WHITE_FIG_FRUITING_LEAVES.get()
                            .getDefaultState()
                            .with(LeavesBlock.DISTANCE, distance)
                            .with(LeavesBlock.PERSISTENT, persistent),
                    3
        ...
        //Fruiting pattern for temperate climates
        if (temp <= 0.89F && "SUMMER".equals(currentSeason) && random.nextInt(55) == 0) {
            int distance = state.get(LeavesBlock.DISTANCE);
            boolean persistent = state.get(LeavesBlock.PERSISTENT);

            worldIn.setBlockState(pos, TreeBlocks.WHITE_FIG_FRUITING_LEAVES.get()
