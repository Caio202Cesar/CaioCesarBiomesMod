...
public class ChileanGuavaFruitingBush extends BushBlock {
    public ChileanGuavaFruitingBush() {
    ...
    public void randomTick(BlockState state, ServerWorld worldIn, BlockPos pos, Random random) {
        ...
        if (random.nextDouble() < chance) {
            ...
            ItemStack itemStack = new ItemStack(ModItems.CHILEAN_GUAVA.get());
            ItemEntity itemEntity = new ItemEntity(worldIn, pos.getX() + 0.5, pos.getY() + 1, pos.getZ() + 0.5, itemStack);

            worldIn.addEntity(itemEntity);

            worldIn.setBlockState(pos, ModPlants.CHILEAN_GUAVA_BUSH.get().getDefaultState());
