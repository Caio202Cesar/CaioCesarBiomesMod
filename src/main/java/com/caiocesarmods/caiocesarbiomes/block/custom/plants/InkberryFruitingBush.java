...
public class InkberryFruitingBush extends BushBlock {
    public InkberryFruitingBush() {
    ...
    public void randomTick(BlockState state, ServerWorld worldIn, BlockPos pos, Random random) {
        ...
        if (random.nextDouble() < chance) {
            worldIn.setBlockState(pos, ModPlants.INKBERRY_BUSH.get().getDefaultState());
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit) {
        if (!worldIn.isRemote) {
            ...
            ItemStack itemStack = new ItemStack(ModItems.INKBERRY.get());
