...
public class ButterMixerContainer extends Container {
    private final TileEntity tileEntity;
    private final PlayerEntity playerEntity;
    private final IItemHandler playerIventory;

    public ButterMixerContainer(int windowId, World world, BlockPos pos, PlayerInventory playerInventory,
                                PlayerEntity player) {

        super(ModContainers.BUTTER_MIXER_CONTAINER.get(), windowId);
        this.tileEntity = world.getTileEntity(pos);
        playerEntity = player;
        this.playerIventory = new InvWrapper(playerInventory);
    ...
    public boolean canInteractWith(PlayerEntity playerIn) {
        return isWithinUsableDistance(IWorldPosCallable.of(tileEntity.getWorld(), tileEntity.getPos()),
                playerIn, ModBlocks.BUTTER_MIXER.get());
    }
    ...
    private void layoutPlayerInventorySlots(int leftCol, int topRow) {
        addSlotBox(playerIventory, 9, leftCol, topRow, 9, 18, 3, 18);

        topRow += 58;
        addSlotRange(playerIventory, 0, leftCol, topRow, 9, 18);
    }
